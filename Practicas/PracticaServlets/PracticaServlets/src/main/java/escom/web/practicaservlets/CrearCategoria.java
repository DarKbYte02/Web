...
@WebServlet(name = "CrearCategoria", urlPatterns = {"/CrearCategoria"})
public class CrearCategoria extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<h2 class='card-title'> Agregar categoría </h2>");
            out.println("</div>");
            out.println("<div class='card-body text-dark'>");
            CategoriaDAO dao = new CategoriaDAO();
            Categoria c = new Categoria();
            out.println("<form action='CrearC'>"
                    + "<div class='row g-3 align-items-center'>"
                    + "<div class='col-auto'>"
                    + "<label class='form-label' style='color: #FFFFFF; font-weight: bold;'>Nombre</label>"
                    + "</div>"
                    + "<div class='col-md-3'>"
                    + "<input type='text' id='nombre' class = 'form-control' name= 'nombre' placeholder='Ingrese el nombre de la categoría' style='background-color: rgb(56,62,67); border-style: none; color: #FFFFFF'/>"
                    + "</div>"
                    + "<div class='col-auto'>"
                    + "<label class='form-label' style='color: #FFFFFF; font-weight: bold;'>Descripción</label>"
                    + "</div>"
                    + "<div class='col-md-7'>"
                    + "<input type='text' id='descripcion' class = 'form-control' name= 'descripcion' placeholder='Ingrese la descripción' style='background-color: rgb(56,62,67); border-style: none; color: #FFFFFF'/>"
                    + "</div>"
                    + "<div>"
